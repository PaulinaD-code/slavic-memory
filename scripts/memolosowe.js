...
let creatures = {
  ...
  BorowaCiotka: {
    name: "Borowa Ciotka",//12 eye-visible
...
checkPoints.addEventListener('click', ()=>{
  ...
  if(sumPoint && wins === loses){
    textPoint.textContent = `Gratuluje. Udało Ci się przejść do kolejnego etapu.`
    level2.className = 'new-opportunity';
  } else if(sumPoint && wins < loses
  ){
    textPoint.textContent = `W grze wykorzystano za dużo powtórnych prób dopasowania kart. Nie udało Ci się przejść do kolejnego etapu.`
  } 
})
